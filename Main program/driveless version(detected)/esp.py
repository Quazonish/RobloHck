...
from numpy import array, float32, reshape, matmul, transpose
            ...
            if not ignoreTeam or (team != lpTeam and team > 0):
                ...
                if head and hum:
                    ...
                    if ignoreDead and health <= 0:
                        continue
                    primitive = pm.read_longlong(head + primitiveOffset)

                    pos = array([
                        pm.read_float(primitive + positionOffset),
                        pm.read_float(primitive + positionOffset + 4),
                        pm.read_float(primitive + positionOffset + 8)
                    ], dtype=float32)
